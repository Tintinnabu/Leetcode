package top.tinn.BitProblem.Problem_756;
...
public class Solution {
    ...
    private boolean dfs(String bottom, List<String> allowed, int index, StringBuilder next) {
        ...
        String prefix = bottom.substring(index, index + 2);
        boolean result ;
        for (int i = 0; i<allowed.size(); i++){
            if (allowed.get(i).startsWith(prefix)){
                next.append(allowed.get(i).charAt(2));
                result = dfs(bottom, allowed, index + 1, next);
    ...
    public void test(){
        String bottom = "AABA";
        List<String> allowed = Arrays.asList("AAA", "AAB", "ABA", "ABB", "BAC");
